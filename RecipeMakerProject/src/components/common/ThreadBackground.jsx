...
export default function ThreadBackground() {
  ...
  useEffect(() => {
    ...
    // ✅ Updated with your desired gradient colors
    const drawBackground = () => {
      const gradient = ctx.createLinearGradient(0, 0, canvas.width, canvas.height);
      gradient.addColorStop(0, "#0c0d14");    // from
      gradient.addColorStop(0.5, "#1d1f31");  // via
      gradient.addColorStop(1, "#0c0d14");    // to
    ...
    const drawThread = (thread, time) => {
      ...
      for (let i = -200; i <= canvas.width + 200; i += step) {
        ...
        const y =
          i * 0.5 +
          thread.offsetY +
          Math.sin((i + time * thread.speed) * 0.005 + thread.phase) *
            thread.amplitude;
        ctx.lineTo(x, y);
      }
